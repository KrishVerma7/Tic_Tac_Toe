...
class MainActivity : AppCompatActivity(), View.OnClickListener{
    ...
    private fun initializeBoardStatus() {
        for(i in 0..2){
            for(j in 0..2){
                boardStatus[i][j] = -1      //setting the value of all buttons as -1
            }
        }
        for(i in board){
            for(button in i){
    ...
    override fun onClick(view: View) {
        ...
        TURN_COUNT++
        PLAYER = !PLAYER        //if true make it false or vice-versa
        ...
        for (i in 0..2) {
            if ((boardStatus[i][0] == boardStatus[i][1]) && (boardStatus[i][0] == boardStatus[i][2])){
                if (boardStatus[i][0] == 1) {
                    ...
                    Toast.makeText(this, "Player Y Winner", Toast.LENGTH_SHORT).show()
                    break
                }
            }
        }
    ...
    private fun updateValue(row: Int, col: Int, player: Boolean) {
        ...
        val text: String = if(player) "X" else "O"      //beauty of Kotlin
        val value: Int = if(player) 1 else 0
        board[row][col].apply{
            isEnabled = false                           //disabling the button so value gets fixed
            setText(text)
        }
